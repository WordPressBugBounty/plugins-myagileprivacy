 ...
 */
if( !class_exists( 'MyAgilePrivacyBannerRenderer' ) )
{
	final class MyAgilePrivacyBannerRenderer
	{
		...
		private function render_banner()
		{
			...
			$_s_bg_color            = ( preg_match( '/^#[0-9a-fA-F]{3,6}$/', $s['heading_background_color'] ) ) ? $s['heading_background_color'] : '';
			$_s_txt_color           = ( preg_match( '/^#[0-9a-fA-F]{3,6}$/', $s['heading_text_color'] ) )        ? $s['heading_text_color']        : '';
			$map_heading_style      = 'background-color:'.$_s_bg_color.'; color: '.$_s_txt_color.';';
			$map_heading_class      = ( $s['title_is_on'] == true ) ? '' : 'map_displayNone';
			$map_close_button_style = ( $s['title_is_on'] == true ) ? 'color: '.$_s_txt_color.'!important;' : 'color: '.esc_attr( $s['text'] ).'!important;';
			$notify_logo_color      = ( $s['heading_background_color'] == '#ffffff' ) ? '#F93F00' : $_s_bg_color;
			$banner_logo_color      = $_s_txt_color;
			...
			$html =
				'<div'.
					...
					' data-animation="'.esc_attr( $s['cookie_banner_animation'] ).'"'.
					' data-wg-notranslate '.
					'>'.
					$notify_title.
					$notify_close_button.
		...
		private function render_cookie_list( $with_css_effects_class )
		{
			...
			foreach( $cookies_categories_data as $k => $v )
			{
				foreach( $v as $key => $value )
				{
					...
					//fallback if post_data->post_title empty
					$cookie_display_title = !empty( $value['post_title'] ) ? $value['post_title'] : $value['post_data']->post_title;

					$cleaned_cookie_name         = str_replace( '"', '', $cookie_display_title );
		...
		)
		{
			...
			$html .= '<a class="map_expandItem map-nav-link map-settings-mobile" data-toggle="map-toggle-tab" role="button" tabindex="0">'.esc_html( $cookie_display_title ).'</a>';
			...
			else
			{
				$html .=
					'<div class="map-switch">'.
						...
						'<div class="map-slider map-for-map-checkbox-'.esc_attr( $the_remote_id ).'" role="checkbox" aria-label="'.esc_attr( $cookie_display_title ).'" tabindex="0" aria-checked="mixed" data-map-enable="'.esc_attr( $is_enabled_text ).'" data-map-disable="'.esc_attr( $is_disabled_text ).'">'.
							'<span class="sr-only">'.esc_html( $cookie_display_title ).'</span>'.
						'</div>'.
					'</div>';
			}
		...
		private function render_custom_css()
		{
		    ...
		    // Inline notify styles
		    $notify_styles = array();

		    $_inline_bg    = ( preg_match( '/^#[0-9a-fA-F]{3,6}$/', $this->the_settings['map_inline_notify_background'] ) ) ? $this->the_settings['map_inline_notify_background'] : '';
		    $_inline_color = ( preg_match( '/^#[0-9a-fA-F]{3,6}$/', $this->the_settings['map_inline_notify_color'] ) )        ? $this->the_settings['map_inline_notify_color']        : '';

		    if( $_inline_bg )
		    {
		        $notify_styles[] = 'background: ' . $_inline_bg . ';';
		    }
		    if( $_inline_color )
		    {
		        $notify_styles[] = 'color: ' . $_inline_color . ';';
		    }
		    if( count( $notify_styles ) > 0 )
		    {
		        $custom_css .= '.map_custom_notify { ' . implode( ' ', $notify_styles ) . ' }';
		    }
